    ...
    <div id="project-card" class="flex flex-col">
      <div id="thumbnail">
        ...
        <div class="absolute top-2 right-2 flex gap-1">
          <div class="px-1.5 py-1 text-[0.75rem] text-white leading-none rounded shadow bg-green-text"
               v-for="tech in project.tech">{{ tech }}
          </div>
        </div>
      </div>
      <div class="pb-8 pt-6 px-6">
        <div class="text-gray-text font-fira_regular text-sm mb-5 line-clamp-2"
             v-html="project.detail.description"></div>
        <a id="view-button" target="_blank"
           class="text-white font-fira_regular py-2 px-4 w-fit text-xs border border-green-text rounded-lg hover:bg-green-text transition-ease cursor-pointer"
    ...
    <WindowModal v-model:visible="detailDlg" header="Window title">
      <template v-slot="{ fullscreen }">
        <div class="w-full flex flex-col md:flex-row gap-10">
          <img class="w-full rounded-sm md:w-1/2 max-w-[400px]" :src="`/projects/${project.img}`"
